...
# algorithms = ["Window"]
# 创建子图
fig, axes = plt.subplots(len(algorithms), 1, figsize=(12, 1.4 * len(algorithms)), dpi=300)
...
for i, algorithm in enumerate(algorithms):
    ...
    # 在子图中绘制时间序列数据
    axes[i].plot(time.flatten(), signal.flatten(), lw=1.6, label='sum_diff', color='#E6756E')

    # 绘制检测到的变化点位置
    for bkp in result:
        if bkp < len(time.flatten()):  # 确保 bkp 在索引范围内
            axes[i].axvline(x=time.flatten()[bkp], color="#1F79BC", linestyle='-.', lw=1.8, label='change points')
    ...
    for j in range(0, len(set_change_times), 2):
        ...
        axes[i].axvline(x=start_time, linestyle='--', color='gray', linewidth=0.8)
        axes[i].axvline(x=end_time, linestyle='--', color='gray', linewidth=0.8)

        color = 'white' if (j+1) % 2 == 0 else '#F0F0F0'  # 每两个区域之间交替使用浅灰色和白色
        axes[i].axvspan(start_time, end_time, facecolor=color, alpha=0.3)
